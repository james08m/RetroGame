...
{
	os << "Objet" << endl << "NOM : " << objet.GetName() << endl << "VIE : " << objet.GetHeal() << endl << "ATTAQUE : " << objet.GetAttaque() << endl << "DEFENSE : " << objet.GetDefense() << endl << endl;
	return os;
}
...
void CObject::Afficher(sf::RenderWindow& Fenetre)
{
	Fenetre.draw(*this);
}
...
void CObject::SetAttaque(int attaque)
{
	Attaque_ = attaque;
}
